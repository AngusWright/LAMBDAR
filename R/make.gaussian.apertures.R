...
#Procedure is parallelised to allow scaleability
  ...
  cat.b.pix<-cat.b/(arcsec.per.pix)
  #}}}
  ...
  gauss2D<-function(xi,yi,x0,y0,asig,bsig,theta) { 
    xy<-rotate.data.2d(xi,yi,theta,about.centre=TRUE)
    zp<-exp(-(((xy[,1]-x0)^2/(2*asig^2))+((xy[,2]-y0)^2/(2*bsig^2))))
    return=cbind(xi,yi,zp)
  }
  #Create Aperture Masks {{{
  message("Creating Aperture Masks")
  s_mask<-foreach(slen=stamplen,axa=theta.offset,maj=cat.a.pix,min=cat.b.pix,xdelt=(cat.x%%1),ydelt=(cat.y%%1),.inorder=TRUE, .options.mpi=mpi.opts) %dopar% {
      ...
      expanded<-gauss2D(xi=grid[,1],yi=grid[,2],x0=ceiling(slen/2)+0.5+xdelt,y0=ceiling(slen/2)+0.5+ydelt,theta=axa,
                          asig=maj,bsig=min)
