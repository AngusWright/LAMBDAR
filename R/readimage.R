...
function(env=NULL, quiet=FALSE, showtime=FALSE, outenv=NULL){ 
  ...
  error<-try(read.fits(paste(pathroot,datamap,sep=""),hdu=extn, comment=FALSE))
  ...
  #Read Data Image
  im_fits<-read.fits(paste(pathroot,datamap,sep=""),hdu=extn, comment=FALSE) 
  ...
  if (errormap=='NONE') { 
    ...
    error<-try(read.fits(paste(pathroot,errormap,sep=""),hdu=extnerr, comment=FALSE))
    if (class(error)=="try error") {
      #Stop on Error
      geterrormessage()
      stop("Error Map File read failed")
    }
    ime_fits=read.fits(paste(pathroot,errormap,sep=""),hdu=extnerr, comment=FALSE)
  ...
  if (maskmap=='NONE') { 
    ...
    error<-try(read.fits(paste(pathroot,maskmap,sep=""),hdu=extnerr, comment=FALSE))
    if (class(error)=="try error") {
      #Stop on Error
      geterrormessage()
      stop("Mask Map read failed")
    }
    imm_fits=read.fits(paste(pathroot,maskmap,sep=""),hdu=extnmask, comment=FALSE)
