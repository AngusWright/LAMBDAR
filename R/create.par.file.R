...
MapUnitsPerBeam        ",paste(MapUnitsPerBeam   ,collapse=' '),"    #Is the Image in units per beam (as opposed to UnitsPerPixel)?            [1/0]
...
CheckContam            ",paste(CheckContam       ,collapse=' '),"    #Check Contaminants for ones that are irrelevant (do not overlap with main galaxies)?  [1/0]
...
PSFConvolve            ",paste(PSFConvolve       ,collapse=' '),"    #Convolve the Aperture with a PSF?                                              [1/0]
...
PSFWeighted            ",paste(PSFWeighted       ,collapse=' '),"    #Do you want PSFWeighted Photometry [1]? or Aperture Photometry [0]?            [1/0]
AngularOffset          ",paste(AngularOffset     ,collapse=' '),"    #0 if the catalogue is in N0E90 angular coords, 1 if it is in N90E0 coords
PointSources           ",paste(PointSources      ,collapse=' '),"    #Force point sources to be used?                                                [1/0]
MinApRad               ",paste(MinApRad          ,collapse=' '),"    #State minimum aperture to use for sources
ResampleAper           ",paste(ResampleAper      ,collapse=' '),"    #Smooth Apertures by resampling? (Improves aperture surface integral accuracy)  [1/0]
...
Verbose                ",paste(Verbose           ,collapse=' '),"    #Verbose Output?
ShowTime               ",paste(ShowTime          ,collapse=' '),"    #Display execution & total elapsed time during run?
PlotSample             ",paste(PlotSample        ,collapse=' '),"    #Plot a sample of the Object Apertures & Fluxes for Inspection? Includes Aperture Images and object COGs
PlotAll                ",paste(PlotAll           ,collapse=' '),"    #Plot All of the Object Apertures & Fluxes for Inspection? Includes Aperture Images and object COGs
Diagnostic             ",paste(Diagnostic        ,collapse=' '),"    #Diagnostic Output of Variable Values During Computation - Helpful in Understanding Code
Interactive            ",paste(Interactive       ,collapse=' '),"    #Interactive Mode - Allows user access to entire parameter-space after final calculations
Magnitudes             ",paste(Magnitudes        ,collapse=' '),"    #Output Fluxes as magnitudes
...
DoSkyEst               ",paste(DoSkyEst          ,collapse=' '),"    #Perform estimate of local sky-background for each object, and subtract it from the flux
...
GetSkyRMS              ",paste(GetSkyRMS         ,collapse=' '),"    #As above without subtraction, and output the local sky RMS (if doing sky estimate, this happens automatically)
RanCor                 ",paste(RanCor            ,collapse=' '),"    #Do you want to calculate mean & median sky-flux using randoms around every aperture? [1/0]
nRandoms               ",paste(nRandoms          ,collapse=' '),"    #Number of randoms calculated *per aperture*
BlankCor               ",paste(BlankCor          ,collapse=' '),"    #Do you want to calculate mean & median confusion using blanks around every aperture? [1/0]
...
IterateFluxes          ",paste(IterateFluxes     ,collapse=' '),"    #Do you want to iterate the flux determination to improve deblending?
...
WriteAAMask            ",paste(WriteAAMask       ,collapse=' '),"    #Write the All Apertures Mask image to file?                                   [1/0]
AllApersFile           ",paste(AllApersFile      ,collapse=' '),"    #Filename for All Apertures Mask
WriteFAMask            ",paste(WriteFAMask       ,collapse=' '),"    #Write the Convolved Apertures Mask image to file?                             [1/0]
ConvApersFile          ",paste(ConvApersFile     ,collapse=' '),"    #Filename for All Convolved Apertures Mask
WriteDFAMask           ",paste(WriteDFAMask      ,collapse=' '),"    #Write the Deblended Convolved Apertures Mask image to file?                   [1/0]
DeblConvApersFile      ",paste(DeblConvApersFile ,collapse=' '),"    #Filename for Deblended Convolved Apertures Mask
WriteSourceMask        ",paste(WriteSourceMask   ,collapse=' '),"    #Write the Source Mask image to file?                                          [1/0]
...
TransmissionMap        ",paste(TransmissionMap   ,collapse=' '),"    #Do you want to produce a Transmission Map instead of a Sourcemask (transparent over sources instead of opaque)? [0/1]
WriteResidMap          ",paste(WriteResidMap     ,collapse=' '),"    #Write the Residual Image Map to file?                                         [1/0]
...
MemorySafe             ",paste(MemorySafe        ,collapse=' '),"    #Do we want to perform checks to ensure the program does not use more memory than is available? [1/0]
