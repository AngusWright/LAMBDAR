...
read.fits.cat<-function(filename,data.table=FALSE,stringsAsFactors=FALSE) { 
  zz <- file(description = filename, open = "rb")
  header0 <- readFITSheader(zz,fixHdr='substitute') # read primary header
  header <- readFITSheader(zz,fixHdr='substitute') # read extension header
